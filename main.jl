...
function run()
    ...
    algs = [importance, importance_corfilter, stepwise, lasso, sparse_encoded_lasso, l1l2, inipg, ga_uniform, ga_twopoint]
        ...
        for ri in eachindex(readers)
            ...
            for i in 1:RVFL_RUNS
                ...
                for cv in eachindex(alldata)
                    ...
                    rvfl = RVFL(length(data.xs), ENHANCED_NODES, MersenneTwister(i))
                    ...
                    nenhanced = JSON.json([sum(collect(s[(end - ENHANCED_NODES + 1):end]) .== '1') for s in selected])
                    selected_json = '[' * join(selected, ',') * ']'
                    coefs = JSON.json(get_coefs(models))
                    nin = length(edata.xs)
                    printheader = RVFL_START == 1 && CV_START == 1 && firstround
